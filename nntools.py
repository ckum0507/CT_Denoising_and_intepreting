...
def load(self):
    ...
    checkpoint = torch.load(self.checkpoint_path, map_location=self.net.device, weights_only=True)
    
    # Make sure the checkpoint is loaded correctly onto the current device
...
class Experiment(object):
    ...
    def __init__(self, net, train_set, val_set, optimizer, stats_manager,
                 ...
                 force_new_experiment=False):  # Add an option to force a new experiment

        # Define data loaders
        train_loader = td.DataLoader(train_set, batch_size=batch_size, shuffle=False,
        ...
        if os.path.isfile(config_path) and not force_new_experiment:  # Skip check if force_new_experiment is True
            with open(config_path, 'r') as f:
                if f.read()[:-1] == repr(self):
                    raise ValueError(
                        "Cannot create this experiment: "
                        "I found a checkpoint conflicting with the current setting.")
            self.load()  # Load previous checkpoint if no conflict and force_new_experiment is False
        else:
            if not os.path.isfile(config_path):  # Only save if there is no previous checkpoint
                self.save()
    ...
    def load(self):
        ...
        checkpoint = torch.load(self.checkpoint_path,
                                map_location=self.net.device, weights_only=True)
        self.load_state_dict(checkpoint)
        del checkpoint
    ...
    def run(self, num_epochs, plot=None):
        ...
        print("Start/Continue training from epoch {}/{}".format(start_epoch, num_epochs))
        if plot is not None:
            plot(self)
